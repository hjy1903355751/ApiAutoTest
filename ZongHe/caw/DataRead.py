...
def readini(filePath, key):
    ...
    :return: key对应的value
    """
    rael_path = getProjectPath() + filePath
    # 调用configparser来解析配置文件
    config = configparser.ConfigParser()
    # 读文件
    config.read(rael_path)
    # env表示section，根据可以在对应的section中去value
...
def readyaml(filePath):
    ...
    with open(real_path, "r", encoding="utf-8") as f:  # 打开文件
        content = yaml.load(f, Loader=yaml.FullLoader)  # 读文件内容，放到变量content中
        return content
